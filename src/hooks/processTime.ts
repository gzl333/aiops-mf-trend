...
let strHours: number | string = myDate.getHours()
let strMinute: number | string = myDate.getMinutes()
...
export const getNowFormatTime = (type: number) => {
  ...
  if (type === 0) {
    ...
    return year + seperator1 + month + seperator1 + strDate + ' ' + strHours + ':' + strMinute
  }
}
...
export function payRecordUtcToBeijingMinuteDetail (utcDatetime: string) {
  // 转为正常的时间格式 年-月-日 时:分:秒
  const newDateTime = utcDatetime.split('T')[0] + ' ' + utcDatetime.split('T')[1].split('.')[0]
  // 处理成为时间戳
  let timeStamp = new Date(newDateTime.replace(/-/g, '/')).getTime()
