...
const axiosAiops = axios.create({
  baseURL: window.location.protocol + '//aiopsbackend.cstcloud.cn/api', // todo 根据需求改成该服务的后端api地址
...
/* 拦截器 */
axiosAiops.interceptors.request.use(config => {
  // get jwt token from @cnic/main's store
  const store = useStoreMain()
  Object.assign(config, { headers: { Authorization: `Bearer ${store.items.tokenAccess as string}` } }) // todo 根据后端格式修改
...
export { axios, axiosAiops, axiosService }
